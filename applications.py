...
def MNISTNet1():
    ...
    model.add(Convolution(conv1_params, name='conv1', initializer=Xavier(9, 72)))
    model.add(ReLU(name='relu1'))
    model.add(Pooling(pool1_params, name='pooling1'))
    model.add(Convolution(conv2_params, name='conv2', initializer=Xavier(288, 32)))
    model.add(ReLU(name='relu2'))
    model.add(Pooling(pool2_params, name='pooling2'))
    model.add(Dropout(ratio=0.25, name='dropout1'))
    model.add(Flatten(name='flatten'))
    model.add(FCLayer(800, 256, name='fclayer1', initializer=Xavier(800, 256))) 
    model.add(ReLU(name='relu3'))
    model.add(Dropout(ratio=0.5))
    model.add(FCLayer(256, 10, name='fclayer2', initializer=Xavier(256, 10)))
    return model
